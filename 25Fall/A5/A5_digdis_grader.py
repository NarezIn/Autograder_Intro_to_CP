...
def grade(chat, prompt_template, n_limit = None):
    ...
    count = 0
    target_dir = uf.get_sub_dir("25Fall", "A5", "dig_dis")
    print("Grading directory:", target_dir)#debug
    rubric = uf.readReturn(os.path.abspath("A5/rubrics/dig_dis_rubric.txt"))
    grade_comment_template = uf.readReturn(os.path.abspath("A5/grade_comment_template.txt"))
...
def digdis_grader(n_limit=None):
    chat = ChatOpenAI(
        model="gpt-5", 
        temperature=0.1,
