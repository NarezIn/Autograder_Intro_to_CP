from A4a_grader import A4a_grader
from A4b_grader import A4b_grader
import re
...
def extract_student_id(filename: str) -> str:
    ...
    """
    match = re.match(r"(.+?)4[a-c]\.py$", filename, re.IGNORECASE)
...
def main():
    graders = [("4a", A4a_grader()), ("4b", A4b_grader())]
    all_results = {}

    for label, grader_func in graders:
        print(f"Running grader {label}...")
        sub_comments = grader_func()#set num of stu to grade
