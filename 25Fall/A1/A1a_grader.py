...
def check_hypotenuse_correct(stu_out, expected=40.608, tol=0.005):
    # extract all numbers from student output
    numbers = re.findall(r"[-+]?\d*\.\d+|\d+", stu_out)
    nums = [float(x) for x in numbers]
    for n in nums:
        if abs(n - expected) <= tol:
            return True, n
    return False, nums[-1] if nums else None
    ...
    for filename in sorted(os.listdir(target_dir), key=str.lower):
        ...
        if filename.endswith(".py"):
            ...
            if not has_comments(stu_code):
                writeInComments(filename, student_grade_comment.content + f"\nHypotenuse: {hypo}, Correct: {correctness}\nIt would be great if you could comment your source code :)")
            else:
                writeInComments(filename, student_grade_comment.content + f"\nHypotenuse: {hypo}, Correct: {correctness}\nCode has comments:)")
